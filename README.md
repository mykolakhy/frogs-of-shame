...
The opt-in signup E2E test also needs a dedicated, disposable `E2E_SIGNUP_EMAIL` address authorized to receive confirmation emails, a server-side Supabase secret key named `SUPABASE_SECRET_KEY`, and `E2E_SIGNUP_TEST_ENABLED=true`. It is used only by the Playwright test runner to delete the user created by the browser flow. Do not reuse `TESTS_USER_EMAIL`, because that account is shared by API/integration tests. Enable the scenario only after configuring a Supabase SMTP provider or authorizing the dedicated address with the project's email service. Never prefix the admin secret with `VITE_`, expose it to frontend code, or commit it to the repository.
